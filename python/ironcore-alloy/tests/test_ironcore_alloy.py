...
class TestIroncoreAlloy:
    ...
    @pytest.mark.asyncio
    async def test_decrypt_vector(self, standalone_sdk):
        ...
        decrypted = await standalone_sdk.vector().decrypt(encrypted_value, metadata)
    ...
    @pytest.mark.asyncio
    async def test_rotate_vector_different_tenant(self, standalone_sdk):
        ciphertext = [5826192.0, 15508204.0, 11420345.0]
        metadata = AlloyMetadata.new_simple("tenant")
        icl_metadata = base64.b64decode(
            b"AAAAAoEACgyVAnirL57DGDIdC28SIH9FFpmMs5yi5CcTcQjcUjldEE0OEdZDWtpyNI++ALnf"
        ...
        metadata = AlloyMetadata.new_simple("tenant")
        rotated = await standalone_sdk.vector().rotate_vectors(
        ...
        decrypted = await standalone_sdk.vector().decrypt(
