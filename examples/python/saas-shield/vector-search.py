...
async def main():
    ...
    plaintext = alloy.PlaintextVector(
        plaintext_vector=data,
        secret_path="contacts",
        derivation_path="conversation-sentiment",
    )
    ...
    search_vector = alloy.PlaintextVector(
        plaintext_vector=[1.4, -1.32, 4.32, 2.37],
        secret_path="contacts",
        derivation_path="conversation-sentiment",
    )
    query_vectors = (
        await sdk.vector().generate_query_vectors({"vec_1": search_vector}, metadata)
    )["vec_1"]
    query_vectors_embeddings = map(
        lambda vector: vector.encrypted_vector, query_vectors
    )
    print("Query vectors:    ", list(query_vectors_embeddings))
    print(
        "Note that the query vectors are a nested list. If this tenant had an in-rotation key, two vectors"
    )
    print(
        "would be in the result. In that case, both vectors must be used in conjunction when querying."
    )
