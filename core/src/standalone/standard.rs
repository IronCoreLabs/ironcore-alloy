...
use crate::document::{self, aes, encrypt_document_core, verify_sig, DocumentError};
...
impl StandardDocumentOps for StandaloneStandardClient {
    ...
    async fn decrypt(
        ...
        if edek_type == Self::get_edek_type() && payload_type == Self::get_payload_type() {
            ...
            Ok(document::decrypt_document_core(
                encrypted_document.document,
                dek,
            )?)
        } else {
            Err(CloakedAiError::InvalidInput(
                "The data indicated that this was not a Standalone Standard wrapped value."
                    .to_string(),
            ))
        }
    }
...
fn decrypt_aes_edek(
...
) -> Result<EncryptionKey, DocumentError> {
    ...
    let aes_edek = maybe_edek_wrapper
        ...
        .ok_or_else(|| DocumentError::DecryptError("No AES EDEK found.".to_string()))?;
...
mod test {
    ...
    fn new_client(primary_secret_id: Option<u32>) -> StandaloneStandardClient {
        StandaloneStandardClient {
            ...
            rng: crate::create_test_seeded_rng(100),
        }
    }
    #[tokio::test]
